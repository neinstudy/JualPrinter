        ...
        <div class="card" style="width: 18rem; margin-right: 25px;">
            ...
            <div class="card-body">
                ...
                <form method="post" action="{{ route('produk.destroy', $row->id) }}">
                    @csrf
                    @method('DELETE')
                    <a href="{{ route('produk.show', $row->id) }}" class="btn btn-primary btn-sm" style="width: 30%">View</a>
                    <a href="{{ route('produk.edit', $row->id) }}" class="btn btn-warning btn-sm" style="width: 30%">Edit</a>
                    <input type="submit" class="btn btn-danger btn-sm" value="Delete" style="width: 30%"/>
                </form>
                </center>
            </div>
        </div>
